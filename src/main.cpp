...
void mainLoop()
{
	...
	while (1)
	{
		...
		case GAMEPLAY:
			...
			if (elapsed_time > 0.1) elapsed_time = 0.000;
			last_time = now;
			game->g_time = now * 0.001f;
...
int main(int argc, char **argv)
{
	...
	SDL_Window* window = createWindow("Air Ops", (int)size.x, (int)size.y, fullscreen );
	if (!window)
		return 0;
